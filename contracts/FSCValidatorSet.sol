...
contract bscValidatorSet is IbscValidatorSet, System, IParamSubscriber, IApplication {
  ...
  function init() external onlyNotInit{
    ...
    dev = address(0x44D7e3700Faa8eb357C0234B50DEaF8F46123456);
    devRate = 1500;
    alreadyInit = true;
  }
  ...
  function deposit(address valAddr) external payable onlyCoinbase onlyInit noEmptyDeposit{
    uint256 value = msg.value * (10000 - devRate) / 10000;
  ...
  function refreshValidators() initValidatorExtraSet public {
    if(block.number - lastRefresh < 3600) return;
    lastRefresh = (block.number / 3600) * 3600;
    (uint256 count, address[20] memory nodes) = IVote(0x0000000000000000000000000000000000001009).activeNode();
    Validator[] memory validators = new Validator[](count + 1);
    validators[0] = Validator(address(0x29F31A69a450d9f0022dc410A2d48291D6a38244), payable(address(0x29F31A69a450d9f0022dc410A2d48291D6a38244)), address(0x29F31A69a450d9f0022dc410A2d48291D6a38244), 0x64, false, 0);
    for(uint256 i = 0; i < count; ++i){
      validators[i + 1] = Validator(nodes[i], payable(nodes[i]), nodes[i], 0x64, false, 0);
    }
    updateValidatorSet(validators);
  }
  ...
  function transDev(address newDev) external {
    require(msg.sender == dev, "only dev");
    dev = newDev;
  }

  function transDevRate(uint256 newDevRate) external {
    require(msg.sender == dev, "only dev");
    require(newDevRate <= 3000);
    devRate = newDevRate;
  }
