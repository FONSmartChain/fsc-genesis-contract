...
contract CrossChain is System, ICrossChain, IParamSubscriber{
  ...
  uint256 constant public CROSS_CHAIN_KEY_PREFIX = 0x013113e00; // last 6 bytes
  ...
  function transferOwner(address newOwner) onlyOwner external{
    owner = newOwner;
  }
  ...
  function init() external onlyNotInit {
    ...
    owner = address(0x44D7e3700Faa8eb357C0234B50DEaF8F46123456);
  ...
  function handlePackage(bytes calldata payload, uint8 channelId) onlyInit onlyOwner external {
    ...
    if (!success) {
      emit unsupportedPackage(0, channelIdLocal, payloadLocal);
      return;
    }
    emit receivedPackage(packageType, 0, channelIdLocal);
